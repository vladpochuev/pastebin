...
const closePopup = (form) => {
    if (form === '#drop-down') {
        ...
        $('.popup-create').fadeOut(200)
        $('.popup-show').fadeOut(200)
...
const showBin = (data) => {
    ...
    $('.info_username').text(trimUsername(data.username))
...
const defineExpirationTime = (time) => {
    ...
    if (days > 0) return days + nbsp + 'day' + definePlural(days)
    else if (hours > 0) return hours + nbsp + 'hour' + definePlural(hours)
    else if (minutes > 0) return minutes + nbsp + 'minute' + definePlural(minutes)
    else if (seconds > 0) return seconds + nbsp + 'second' + definePlural(seconds)
}
